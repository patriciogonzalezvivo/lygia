...
inline __host__ __device__ float map( float value, float inMin, float inMax ) {
    return saturate( (value-inMin)/(inMax-inMin));
}

inline __host__ __device__ float2 map( float2 value, float2 inMin, float2 inMax ) {
    return saturate( (value-inMin)/(inMax-inMin));
}

inline __host__ __device__ float3 map( float3 value, float3 inMin, float3 inMax ) {
    return saturate( (value-inMin)/(inMax-inMin));
}

inline __host__ __device__ float4 map( float4 value, float4 inMin, float4 inMax ) {
    return saturate( (value-inMin)/(inMax-inMin));
}
