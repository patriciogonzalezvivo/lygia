...
use: <float> raymarchSoftshadow( in <float3> ro, in <float3> rd, in <float> mint, in <float> tmax) 
*/

#ifndef FNC_RAYMARCHSOFTSHADOW
#define FNC_RAYMARCHSOFTSHADOW

float raymarchSoftShadow( in float3 ro, in float3 rd, in float mint, in float tmax ) {
    float res = 1.0;
    float t = mint;
    for ( int i=0; i<16; i++ ) {
