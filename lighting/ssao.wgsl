#include "../math/saturate.wgsl"
#include "../space/linearizeDepth.wgsl"
#include "../space/depth2viewZ.wgsl"
#include "../sampler.wgsl"
#include "../generative/random.wgsl"

/*
contributors: Patricio Gonzalez Vivo
description: ScreenSpace Ambient Occlusion
use: <float> ssao(<SAMPLER_TYPE> texPosition, <SAMPLER_TYPE> texNormal, vec2 <st> [, <float> radius, float <bias>])
options:
    - SSAO_SAMPLES_NUM: number of half-sphere offsets samples
    - SSAO_SAMPLES_ARRAY: array of weighted vec3 half-sphere offsets
    - SSAO_NOISE_NUM: number of vec3 noise offsets
    - SSAO_NOISE_ARRAY: array of vec3 noise offsets
    - SSAO_NOISE2_FNC(ST): (random2(ST * 100.) * 0.1)
    - SSAO_NOISE3_FNC(POS): (random3(POS) * 0.3)
    - CAMERA_PROJECTION_MATRIX: camera projection mat4 matrix
    - CAMERA_NEAR_CLIP: required for depth only SSAO
    - CAMERA_FAR_CLIP: required for depth only SSAO
    - SAMPLER_FNC(TEX, UV): optional depending the target version of GLSL (texture2D(...) or texture(...))
license:
    - Copyright (c) 2021 Patricio Gonzalez Vivo under Prosperity License - https://prosperitylicense.com/versions/3.0.0
    - Copyright (c) 2021 Patricio Gonzalez Vivo under Patron License - https://lygia.xyz/license
*/

const SSAO_SAMPLES: f32 = 60.0;

const SSAO_SAMPLES_NUM: f32 = 8;

// #define SSAO_SAMPLES_ARRAY u_ssaoSamples
uniform vec3 u_ssaoSamples[SSAO_SAMPLES_NUM];

const SSAO_NOISE_NUM: f32 = 4;

// #define SSAO_NOISE_ARRAY u_ssaoNoise
uniform vec3 u_ssaoNoise[SSAO_NOISE_NUM];

// #define SSAO_NOISE2_FNC(ST) random2(ST)

// #define SSAO_NOISE3_FNC(POS) random3(POS)

const SSAO_DEPTH_BIAS: f32 = 0.05;

// #define SSAO_DEPTH_SAMPLE_FNC(TEX, UV) SAMPLER_FNC(TEX, UV).r

// #define SSAO_POS_SAMPLE_FNC(TEX, UV) SAMPLER_FNC(TEX, UV)

// #define SSAO_NORMAL_SAMPLE_FNC(TEX, UV) SAMPLER_FNC(TEX, UV).xyz

fn ssao(texDepth: SAMPLER_TYPE, st: vec2f, pixel: vec2f, radius: f32) -> f32 {

    let noiseS = sqrt(float(SSAO_NOISE_NUM));
    let noiseX = int( mod(gl_FragCoord.x - 0.5, noiseS) );
    let noiseY = int( mod(gl_FragCoord.y - 0.5, noiseS) );
    let noise = SSAO_NOISE_ARRAY[noiseX + noiseY * int(noiseS)].xy;
    let noise = SSAO_NOISE2_FNC( st );
    noise *= 0.1;

    let depth = depth2viewZ( SSAO_DEPTH_SAMPLE_FNC( texDepth, st ) ) + SSAO_DEPTH_BIAS * 0.5;
    let ao = 0.0;

    // if (depth < 0.99) 
    {
        let w = pixel.x / depth + noise.x;
        let h = pixel.y / depth + noise.y;

        let dz = 1.0 / float( SSAO_SAMPLES_NUM );
        let l = 0.0;
        let z = 1.0 - dz * 0.5;

        for ( int i = 0; i < SSAO_SAMPLES_NUM; i ++ ) { 
            let r = sqrt( 1.0 - z );
            let pw = cos( l ) * r;
            let ph = sin( l ) * r;
            let vv = radius * vec2f( pw * w, ph * h);
            ao += ( step( depth2viewZ( SSAO_DEPTH_SAMPLE_FNC( texDepth, st + vv) ), depth) + 
                    step( depth2viewZ( SSAO_DEPTH_SAMPLE_FNC( texDepth, st - vv) ), depth) ) * 0.5;
            z = z - dz; 
            l = l + 2.399963229728653; 
        } 
        ao = 1.0 - ao * dz;
        ao = saturate( 1.98 * ( 1.0 - ao ) );
    }
    return ao;
}

fn ssaoa(texDepth: sampler2D, coord: vec2f) -> f32 {
    let cd = SSAO_DEPTH_SAMPLE_FNC(texDepth, coord);
    let ao_radious = 1.0/100.0;
    let screenRadius = 0.5 * (ao_radious / cd) / 0.53135;
    let li = 0.0;
    let count = 0.0;
    for(float i=0.0; i< SSAO_SAMPLES; i++) {
        let p = normalize(SSAO_NOISE3_FNC(vec3f(coord, i)) * 2.0 - 1.0);
        let sp = vec2f(coord.x + p.x * screenRadius, coord.y + p.y * screenRadius);
        let d = SSAO_DEPTH_SAMPLE_FNC(texDepth, sp);
        let at = pow(length(p)-1.0, 2.0);
        li += step(cd + p.z * ao_radious, d) * at;
        count += at;
    }
    return 1.0 - li / count;
}

fn ssaob(texPosition: SAMPLER_TYPE, texNormal: SAMPLER_TYPE, st: vec2f, radius: f32) -> f32 {
    let position = SSAO_POS_SAMPLE_FNC(texPosition, st);
    let normal = SSAO_NORMAL_SAMPLE_FNC(texNormal, st);

    let noiseS = sqrt(float(SSAO_NOISE_NUM));
    let noiseX = int( mod(gl_FragCoord.x - 0.5, noiseS) );
    let noiseY = int( mod(gl_FragCoord.y - 0.5, noiseS) );
    let noise = SSAO_NOISE_ARRAY[noiseX + noiseY * int(noiseS)];
    let noise = SSAO_NOISE3_FNC( position.xyz );

    let tangent = normalize(noise - normal * dot(noise, normal));
    let binormal = cross(normal, tangent);
    let tbn = mat3x3<f32>(tangent, binormal, normal);

    let occlusion = 0.0;
    for (int i = 0; i < SSAO_SAMPLES_NUM; ++i) {
        let samplePosition = tbn * SSAO_SAMPLES_ARRAY[i];
        samplePosition = position.xyz + samplePosition * radius;

        let offsetUV = vec4f(samplePosition, 1.0);
        offsetUV = CAMERA_PROJECTION_MATRIX * offsetUV;
        offsetUV.xy /= offsetUV.w;
        offsetUV.xy = offsetUV.xy * 0.5 + 0.5;

        let sampleDepth = SAMPLER_FNC(texPosition, offsetUV.xy).z;
        let rangeCheck = smoothstep(0.0, 1.0, radius / abs(position.z - sampleDepth));
        occlusion += (sampleDepth >= samplePosition.z + SSAO_DEPTH_BIAS ? 1.0 : 0.0) * rangeCheck;
    }

    occlusion /= float(SSAO_SAMPLES_NUM);
    return 1.0-occlusion;
}
