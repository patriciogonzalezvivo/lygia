...
use: <bool> AABBexpand(<AABB> box, <AABB|vec3|float> point ) 
*/

#ifndef FNC_AABB_EXPAND
#define FNC_AABB_EXPAND

void AABBexpand(inout AABB _box, const in float _value ) {
    _box.min -= _value;
    _box.max += _value;
}

void AABBexpand(inout AABB _box, const in vec3 _point ) {
...
void AABBexpand(inout AABB _box, const in AABB _other ) {
