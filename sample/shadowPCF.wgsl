...
fn sampleShadowPCF(depths: SAMPLER_TYPE, size: vec2f, uv: vec2f, compare: f32) -> f32 {
    let pixel = 1.0/size;
    let result = 0.0;
    for (float x= -2.0; x <= 2.0; x++)
        for (float y= -2.0; y <= 2.0; y++) 
            result += SAMPLESHADOWPCF_SAMPLER_FNC(depths, size, uv + vec2f(x,y) * pixel, compare);
    return result/25.0;
}
