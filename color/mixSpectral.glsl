...
void mixSpectral_upsampling(vec3 lrgb, out float w, out float c, out float m, out float y, out float r, out float g, out float b) {
...
void mixSpectral_linear_to_reflectance(vec3 lrgb, inout float R[MIXSPECTRAL_SIZE]) {
    ...
    mixSpectral_upsampling(lrgb, w, c, m, y, r, g, b);
