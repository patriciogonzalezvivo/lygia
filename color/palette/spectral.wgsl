#include "spectral/gems.wgsl"
#include "spectral/geoffrey.wgsl"
#include "spectral/soft.wgsl"
#include "spectral/zucconi.wgsl"
#include "spectral/zucconi6.wgsl"
...
fn spectrala(x: f32, l: f32) -> vec3f {
    x = 1.0 - x;
    ...
    // GREEN, BLUE
    .5 + .5 * cos( PI * min(
    vec2f( 1. ), abs( vec2f( x ) - vec2f( .5, ( 1.0 - ( ( 2. + l ) / 3. ) * .5 ) ) )
    * vec2f( 3. + l ) ) ) );
}
