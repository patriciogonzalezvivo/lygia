...
use: <float4> layerScreenSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerScreenSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute screen for RGB channels
    float3 blendedColor = blendScreen(src.rgb, dest.rgb);
