...
use: <float4> layerSoftLightSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerSoftLightSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute soft light for RGB channels
    float3 blendedColor = blendSoftLight(src.rgb, dest.rgb);
