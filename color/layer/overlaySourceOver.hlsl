...
use: <float4> layerOverlaySourceOver(<float4> src, <float4> dst)
license:
...
float4 layerOverlaySourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute overlay for RGB channels
    float3 blendedColor = blendOverlay(src.rgb, dest.rgb);
