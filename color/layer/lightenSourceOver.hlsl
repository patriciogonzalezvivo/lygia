...
use: <float4> layerLightenSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerLightenSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute lighten for RGB channels
    float3 blendedColor = blendLighten(src.rgb, dest.rgb);
