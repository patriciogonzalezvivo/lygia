...
use: <float4> layerNegationSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerNegationSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute negation for RGB channels
    float3 blendedColor = blendNegation(src.rgb, dest.rgb);
