...
use: <float4> layerLinearBurnSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerLinearBurnSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute linear burn for RGB channels
    float3 blendedColor = blendLinearBurn(src.rgb, dest.rgb);
