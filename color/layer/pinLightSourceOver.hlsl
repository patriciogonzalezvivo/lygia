...
use: <float4> layerPinLightSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerPinLightSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute pin light for RGB channels
    float3 blendedColor = blendPinLight(src.rgb, dest.rgb);
