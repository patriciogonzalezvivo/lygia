...
use: <float4> layerSubtractSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerSubtractSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute subtract for RGB channels
    float3 blendedColor = blendSubtract(src.rgb, dest.rgb);
