...
use: <float4> layerAverageSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerAverageSourceOver(float4 src, float4 dest) {    
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute average for RGB channels
    float3 blendedColor = blendAverage(src.rgb, dest.rgb);
