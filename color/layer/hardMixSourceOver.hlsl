...
use: <float4> layerHardMixSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerHardMixSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute hard mix for RGB channels
    float3 blendedColor = blendHardMix(src.rgb, dest.rgb);
