...
use: <float4> layerColorBurnSourceOver(<float4> src, <float4> dst)
license:
...
float4 layerColorBurnSourceOver(float4 src, float4 dest) {
    float4 result = float4(0.0, 0.0, 0.0, 0.0);

    // Compute color burn for RGB channels
    float3 blendedColor = blendColorBurn(src.rgb, dest.rgb);
