...
#include "pyramid/downscale.wgsl"
#include "pyramid/upscale.wgsl"
...
fn pyramid(tex0: SAMPLER_TYPE, tex1: SAMPLER_TYPE, st: vec2f, pixel: vec2f, upscale: bool) -> vec4f {
    let color = vec4f(0.0);
    if (!upscale) {
    ...
    return (color.a == 0.0)? color : vec4f(color.rgb/color.a, 1.0);
}
