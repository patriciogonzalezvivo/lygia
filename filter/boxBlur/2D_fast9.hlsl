...
BOXBLUR2D_FAST9_TYPE boxBlur2D_fast9(in sampler2D tex, in float2 st, in float2 offset) {
    ...
    color += BOXBLUR2D_FAST9_SAMPLER_FNC(st + float2(-offset.x, offset.y));  // tleft
    color += BOXBLUR2D_FAST9_SAMPLER_FNC(st + float2(-offset.x, 0.));        // left
    color += BOXBLUR2D_FAST9_SAMPLER_FNC(st + float2(-offset.x, -offset.y)); // bleft
    color += BOXBLUR2D_FAST9_SAMPLER_FNC(st + float2(0., offset.y));         // top
    color += BOXBLUR2D_FAST9_SAMPLER_FNC(st + float2(0., -offset.y));        // bottom
    color += BOXBLUR2D_FAST9_SAMPLER_FNC(st + offset);                     // tright
    color += BOXBLUR2D_FAST9_SAMPLER_FNC(st + float2(offset.x, 0.));         // right
    color += BOXBLUR2D_FAST9_SAMPLER_FNC(st + float2(offset.x, -offset.y));  // bright
