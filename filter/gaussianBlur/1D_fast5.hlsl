...
use: gaussianBlur1D_fast5(<sampler2D> texture, <float2> st, <float2> pixel_direction)
options:
...
GAUSSIANBLUR1D_FAST5_TYPE gaussianBlur1D_fast5(in sampler2D tex, in float2 st, in float2 offset) {
    float2 off1 = float2(1.3333333333333333, 1.3333333333333333) * offset;
    GAUSSIANBLUR1D_FAST5_TYPE color = GAUSSIANBLUR1D_FAST5_SAMPLER_FNC(st) * .29411764705882354;
