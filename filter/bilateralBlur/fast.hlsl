...
use: bilateralBlurFast(<sampler2D> texture, <float2> st, <float2> pixelSize, <float> smoothingFactor,  <float> kernelSize)
options:
    - BILATERALBLURFAST_TYPE: default is float4
...
#else
#define BILATERALBLURFAST_TYPE float4
#endif
#endif
...
#define BILATERALBLURFAST_NOISE_FNC(TEX, UV) SAMPLER_FNC(poissonNoise, nearest(frac(TEX, UV),float2(BILATERALBLURFAST_NOISE_TEX_SIZE))).xy
#endif
...
BILATERALBLURFAST_TYPE bilateralBlurFast(in sampler2D tex, in float2 st, in float2 pixel, in float smoothingFactor, const float sigma_s) {
    ...
    for (float i = 0.0; i < numSamples && accumWeight <= .25 * numSamples; i++) {
        float2 coords = BILATERALBLURFAST_NOISE_FNC(float2(i * samplePitch, yFetch));
...
BILATERALBLURFAST_TYPE bilateralBlurFast10(in sampler2D tex, in float2 st, in float2 pixel, in float smoothingFactor) {
    BILATERALBLURFAST_TYPE colorRef = BILATERALBLURFAST_SAMPLER_FNC(tex, st);
    ...
    for (float i = 0.0; i < 10. && accumWeight <= 2.5; i++) {
        float2 coords = BILATERALBLURFAST_NOISE_FNC(float2(i * .1, yFetch));
        coords = (coords - .5) * scale;
        float coordsz = dot(coords, coords);
        BILATERALBLURFAST_TYPE colorFetch = BILATERALBLURFAST_SAMPLER_FNC(tex, coords + st);
        BILATERALBLURFAST_TYPE colorDist = colorFetch - colorRef;
...
BILATERALBLURFAST_TYPE bilateralBlurFast20(in sampler2D tex, in float2 st, in float2 pixel, in float smoothingFactor) {
    BILATERALBLURFAST_TYPE colorRef = BILATERALBLURFAST_SAMPLER_FNC(tex, st);
    BILATERALBLURFAST_TYPE accumColor = BILATERALBLURFAST_TYPE(0.);
    float accumWeight = 0.;
    ...
    for (float i = 0.0; i < 20. && accumWeight <= 5.; i++) {
        float2 coords = BILATERALBLURFAST_NOISE_FNC(float2(i * 0.05, yFetch));
        coords = (coords - .5) * scale;
        float coordsz = dot(coords, coords);
        BILATERALBLURFAST_TYPE colorFetch = BILATERALBLURFAST_SAMPLER_FNC(tex, coords + st);
        BILATERALBLURFAST_TYPE colorDist = colorFetch - colorRef;
...
BILATERALBLURFAST_TYPE bilateralBlurFast30(in sampler2D tex, in float2 st, in float2 pixel, in float smoothingFactor) {
    BILATERALBLURFAST_TYPE colorRef = BILATERALBLURFAST_SAMPLER_FNC(tex, st);
    BILATERALBLURFAST_TYPE accumColor = BILATERALBLURFAST_TYPE(0.);
    float accumWeight = 0.;
    ...
    for (float i = 0.0; i < 30. && accumWeight <= 7.5; i++) {
        float2 coords = BILATERALBLURFAST_NOISE_FNC(float2(i * .03333333333, yFetch));
        coords = (coords - .5) * scale;
        float coordsz = dot(coords, coords);
        BILATERALBLURFAST_TYPE colorFetch = BILATERALBLURFAST_SAMPLER_FNC(tex, coords + st);
        BILATERALBLURFAST_TYPE colorDist = colorFetch - colorRef;
...
BILATERALBLURFAST_TYPE bilateralBlurFast40(in sampler2D tex, in float2 st, in float2 pixel, in float smoothingFactor) {
    BILATERALBLURFAST_TYPE colorRef = BILATERALBLURFAST_SAMPLER_FNC(tex, st);
    BILATERALBLURFAST_TYPE accumColor = BILATERALBLURFAST_TYPE(0.);
    float accumWeight = 0.;
    ...
    for (float i = 0.0; i < 40. && accumWeight <= 10.; i++) {
        float2 coords = BILATERALBLURFAST_NOISE_FNC(float2(i * .025, yFetch));
        coords = (coords - .5) * scale;
        float coordsz = dot(coords, coords);
        BILATERALBLURFAST_TYPE colorFetch = BILATERALBLURFAST_SAMPLER_FNC(tex, coords + st);
        BILATERALBLURFAST_TYPE colorDist = colorFetch - colorRef;
...
BILATERALBLURFAST_TYPE bilateralBlurFast50(in sampler2D tex, in float2 st, in float2 pixel, in float smoothingFactor) {
    BILATERALBLURFAST_TYPE colorRef = BILATERALBLURFAST_SAMPLER_FNC(tex, st);
    BILATERALBLURFAST_TYPE accumColor = BILATERALBLURFAST_TYPE(0.);
    float accumWeight = 0.;
    ...
    for (float i = 0.0; i < 50. && accumWeight <= 12.5; i++) {
        float2 coords = BILATERALBLURFAST_NOISE_FNC(float2(i * .02, yFetch));
        coords = (coords - .5) * scale;
        float coordsz = dot(coords, coords);
        BILATERALBLURFAST_TYPE colorFetch = BILATERALBLURFAST_SAMPLER_FNC(tex, coords + st);
        BILATERALBLURFAST_TYPE colorDist = colorFetch - colorRef;
...
BILATERALBLURFAST_TYPE bilateralBlurFast60(in sampler2D tex, in float2 st, in float2 pixel, in float smoothingFactor) {
    BILATERALBLURFAST_TYPE colorRef = BILATERALBLURFAST_SAMPLER_FNC(tex, st);
    BILATERALBLURFAST_TYPE accumColor = BILATERALBLURFAST_TYPE(0.);
    float accumWeight = 0.;
    ...
    for (float i = 0.0; i < 60. && accumWeight <= 15.; i++) {
        float2 coords = BILATERALBLURFAST_NOISE_FNC(float2(i * .01666666667, yFetch));
        coords = (coords - .5) * scale;
        float coordsz = dot(coords, coords);
        BILATERALBLURFAST_TYPE colorFetch = BILATERALBLURFAST_SAMPLER_FNC(tex, coords + st);
        BILATERALBLURFAST_TYPE colorDist = colorFetch - colorRef;
