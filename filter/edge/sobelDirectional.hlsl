...
use: edgeSobel_directional(<sampler2D> texture, <float2> st, <float2> pixels_scale)
options:
...
#else
#define EDGESOBELDIRECTIONAL_SAMPLER_FNC(POS_UV) SAMPLER_FNC(tex, POS_UV).r
#endif
#endif

#ifndef FNC_EDGESOBEL_DIRECTIONAL
#define FNC_EDGESOBEL_DIRECTIONAL
float3 edgeSobelDirectional(in sampler2D tex, in float2 st, in float2 offset) {
    // get samples around pixel
    float tleft = EDGESOBELDIRECTIONAL_SAMPLER_FNC(st + float2(-offset.x, offset.y));
    float left = EDGESOBELDIRECTIONAL_SAMPLER_FNC(st + float2(-offset.x, 0.));
    float bleft = EDGESOBELDIRECTIONAL_SAMPLER_FNC(st + float2(-offset.x, -offset.y));
    float top = EDGESOBELDIRECTIONAL_SAMPLER_FNC(st + float2(0., offset.y));
    float bottom = EDGESOBELDIRECTIONAL_SAMPLER_FNC(st + float2(0., -offset.y));
    float tright = EDGESOBELDIRECTIONAL_SAMPLER_FNC(st + offset);
    float right = EDGESOBELDIRECTIONAL_SAMPLER_FNC(st + float2(offset.x, 0.));
    float bright = EDGESOBELDIRECTIONAL_SAMPLER_FNC(st + float2(offset.x, -offset.y));
